...
inline float dotproduct(glm::vec4 u, glm::vec4 v)
{
    ...
    if ( u4 != 0.0f || v4 != 0.0f )
    {
        ...
        if (!warned) {
            fprintf(stderr, "WARNING: dotproduct called with a point (w!=0). Ignoring w and computing xyz dot.\n");
            warned = true;
        }
        // Treat points as vectors by ignoring the homogeneous coordinate.
    }
